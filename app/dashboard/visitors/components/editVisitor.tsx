...
import { INSERT_VISITS, INSERT_VISITS_VISITOR, UPDATE_VISITOR, UPDATE_VISITS } from '../mutation/insert_visits';
import toast from 'react-hot-toast';

export default function EditVisitor({opened, close, data}: any) {
    const form = useForm({
        mode: 'uncontrolled',
        initialValues: {
          firstname: null,
        ...
        useEffect(() =>{
            ...
            form.setFieldValue('firstname', data?.visitorByVisitor?.firstname)
            form.setFieldValue('lastname', data?.visitorByVisitor?.lastname)
            form.setFieldValue('id_card_number', data?.visitorByVisitor?.id_number)
            form.setFieldValue('phone_number', data?.visitorByVisitor?.phone_number)
            form.setFieldValue('reason', data?.reason)
            // department
            form.setFieldValue('employee', data?.employee?.id)
            form.setFieldValue('department', data?.department?.id)
            form.setFieldValue('service', data?.service?.id)
            form.setFieldValue('vehicle', data?.vehicleByVehicle?.id)
            form.setFieldValue('visitors', data?.visitorByVisitor?.id)
    ...
    const [updateVisitor, {data: dataVisitWVisitor, loading: loadVWV, error: errVWV}] = useMutation(UPDATE_VISITOR)
    const [updateVisits, {data: dataVisit, loading: loadVisit, error: errVisit}] = useMutation(UPDATE_VISITS)
  ...
  return (
    <>
      <Modal opened={opened} size="lg" onClose={close} title= {<p style={{color: "#404040"}}> Edit Visitor </p>}>
        {/* Modal content */}
